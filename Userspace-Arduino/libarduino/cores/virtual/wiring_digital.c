...
uint8_t getPinMode(uint8_t pin)
{
	if (g_APinDescription[pin].pinType != ANALOG) {
        gpio_export(g_APinDescription[pin].gpioPin);
    return gpio_getdirection(g_APinDescription[pin].gpioPin);
	}
	return -1;
}
...
void pinMode(uint8_t pin, uint8_t mode)
{
	...
	*/
	if ((g_APinDescription[pin].pinType != ANALOG) 
        && (g_APinDescription[pin].pinType != LED) 
        )
    {
		if (mode == INPUT)
			gpio_setdirection(g_APinDescription[pin].gpioPin, "in");
...
void digitalWrite(uint8_t pin, uint8_t val)
{
    ...
    */
    if (g_APinDescription[pin].pinType == LED) {
		sysfs_led_setvalue((g_APinDescription[pin].headerPin - 21), val);
    } else if ((g_APinDescription[pin].pinType != ANALOG)
        && (g_APinDescription[pin].pinType != _SPI)
        ) {
		sysfs_gpio_setvalue(g_APinDescription[pin].gpioPin, val);
    } else
		return;
